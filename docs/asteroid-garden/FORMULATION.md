...
- 羊は地球モードだけの移動する放牧粒子として扱う。羊は若い芽を摘み、食べた炭素の一部を近傍土壌の炭素・無機養分へ戻す。水域・海岸を越えて移動しない。
...
採食対象は若い芽に限る。バオバブ、バラの表示質量をそれぞれ $m^B_i,m^R_i$ とし、しきい値 $m^{B,g}_{\max},m^{R,g}_{\max}$ 以下の若い個体だけを可食量とする。

$$
\begin{aligned}
G^B_i &= m^B_i\,\mathbf{1}\left(m^B_{\min}<m^B_i\le m^{B,g}_{\max}\right),\\
G^R_i &= m^R_i\,\mathbf{1}\left(0<m^R_i\le m^{R,g}_{\max}\right).
\end{aligned}
$$
...
である。実装ではバオバブの若芽を先に摘み、残った作業量でバラの若芽を摘む。除去された植物炭素を $C^{eat}_{q,i}$ とすると、その一部は近傍セル $d_q(i)$ のリター・土壌側へ戻り、無機養分も増える。
...
戻らない分は、羊の代謝・体内保持・ゲーム外への輸送をまとめた外部項として扱う。
...
無機養分は単独では保存量ではない。鉱物化・有機窒素放出・風化・火山灰風化・放牧糞由来の還元は無機養分への入力、植物吸収・リーチングは無機養分からの出力である。水平輸送だけは全球和を変えない。
